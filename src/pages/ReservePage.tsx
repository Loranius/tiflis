...
function effectiveLabels(reservation: Reservation, tableById: Map<number, ReserveTable>): string[] {
...
export function ReservePage() {
  ...
  const hallById = useMemo(() => new Map(halls.map((hall) => [hall.id, hall])), [halls]);
  const activeHall = halls.find((hall) => hall.id === activeHallId) || null;
  const hallTables = useMemo(
    () => tables.filter((table) => table.hall_id === activeHallId).sort(naturalTableSort),
    [activeHallId, tables],
  );
  const selectedReservations = useMemo(
    () => reservations.filter((reservation) => reservation.reserved_date === selectedDate),
  ...
  const reservationsByTable = useMemo(() => {
    ...
    hallReservations.forEach((reservation) => {
      effectiveLabels(reservation, tableById).forEach((label) => {
        map.set(label, [...(map.get(label) || []), reservation]);
      });
    });
    return map;
  ...
  const metrics = useMemo(() => {
    const activeAllHalls = selectedReservations.filter(isActiveReservation);
    const activeHallReservations = hallReservations.filter(isActiveReservation);
    const guests = activeAllHalls.reduce((sum, reservation) => sum + reservation.guests, 0);
    const occupied = activeAllHalls.filter((reservation) => reservation.status === 'occupied').length;
    const available = hallTables.filter((table) => {
      return !(reservationsByTable.get(table.label) || []).some(isActiveReservation);
    }).length;
    return { reservations: activeAllHalls.length, guests, occupied, available, hallActive: activeHallReservations.length };
  }, [hallReservations, hallTables, reservationsByTable, selectedReservations]);
  ...
  const floorBounds = useMemo(() => {
    ...
    return { maxX: Math.max(1, ...xs) + 1, maxY: Math.max(1, ...ys) + 1 };
  }, [hallTables]);
  ...
  const editorTables = useMemo(
    () => tables.filter((table) => table.hall_id === editor?.hallId).sort(naturalTableSort),
    [editor?.hallId, tables],
  );
  const editorPrimary = editorTables.find((table) => table.id === editor?.tableId) || null;
  const editorCapacity = editorTables
    .filter((table) => table.id === editor?.tableId || Boolean(editor?.mergedTables.includes(table.label)))
    .reduce((sum, table) => sum + table.capacity, 0);

  function tableTone(table: ReserveTable): 'free' | 'booked' | 'occupied' {
    const bookings = reservationsByTable.get(table.label) || [];
    if (bookings.some((reservation) => reservation.status === 'occupied')) return 'occupied';
    if (bookings.some((reservation) => reservation.status === 'booked')) return 'booked';
    return 'free';
  }
  ...
  return (
    <div className="reserve-page-v2">
          ...
          {dateStrip.map((date) => {
            ...
            return (
              <button
                type="button"
                key={date}
                className={`${date === selectedDate ? 'is-selected' : ''} ${date === localIso() ? 'is-today' : ''}`}
                onClick={() => setSelectedDate(date)}
              >
                <span>{DOW[parsed.getDay()]}</span>
                <strong>{parsed.getDate()}</strong>
                <small>{MONTH_SHORT[parsed.getMonth()]}</small>
                {countsByDate.get(date) ? <i>{countsByDate.get(date)}</i> : null}
              </button>
            );
          })}
        </div>
      ...
      <section className="reserve-metrics-v2">
        ...
        <article><Check size={19} /><span>Вільних у цьому залі</span><strong>{metrics.available}</strong></article>
      </section>

      <section className="reserve-workspace-v2">
        <div className="reserve-workspace-head-v2">
          <div>
            <span className="eyebrow">{formatDate(selectedDate)}</span>
            <h3>{activeHall?.name || 'Зали ресторану'}</h3>
            <p>{activeHall?.note || `${metrics.hallActive} активних резервів у цьому залі`}</p>
          </div>
          <div className="reserve-head-actions-v2">
            <div className="reserve-view-switch-v2">
              <button type="button" className={view === 'floor' ? 'is-active' : ''} onClick={() => setView('floor')}><LayoutGrid size={16} /> Схема</button>
              <button type="button" className={view === 'timeline' ? 'is-active' : ''} onClick={() => setView('timeline')}><List size={16} /> Таймлайн</button>
            </div>
        ...
        {!loading && view === 'floor' ? (
          <div className="reserve-floor-wrap-v2">
            ...
            <div className="reserve-floor-canvas-v2" style={{ aspectRatio: `${Math.max(1.8, floorBounds.maxX / floorBounds.maxY)}` }}>
              {hallTables.map((table) => {
                ...
                const first = (reservationsByTable.get(table.label) || []).find(isActiveReservation);
                ...
                return (
                  <button
                    type="button"
                    key={table.id}
                    className={`reserve-floor-table-v2 ${tableSizeClass(table.kind)} tone-${tone}`}
                    style={{ left: `${left}%`, top: `${top}%` }}
                    onClick={() => setFocusedTable(table)}
                  >
                    <strong>{table.label}</strong>
                    <span>{table.capacity} ос.</span>
                    <small>{first ? shortTime(first.reserved_time) : 'вільний'}</small>
                    {(first?.merged_tables || []).length > 0 ? <i>+{first?.merged_tables?.length}</i> : null}
                  </button>
                );
              })}
            </div>
        ...
        {!loading && view === 'timeline' ? (
          <div className="reserve-timeline-v2">
            ...
            {filteredTimeline.map((reservation) => {
              ...
              return (
                <article key={reservation.id} className={`tone-${status.tone}`}>
                  <time>{shortTime(reservation.reserved_time)}</time>
                  <div className="reserve-timeline-main-v2">
                    <div><strong>{reservation.guest_name}</strong><span><UsersRound size={13} /> {reservation.guests} · стіл {table?.label || '—'}{(reservation.merged_tables || []).length ? ` + ${reservation.merged_tables?.join(', ')}` : ''}</span></div>
                    <p>{reservation.phone || reservation.menu_note || reservation.note || 'Без додаткових приміток'}</p>
                  </div>
                  <span className="reserve-status-v2">{status.label}</span>
                  {data?.permissions.canManage ? <button type="button" onClick={() => setEditor(createEditor(selectedDate, reservation.hall_id, reservation.table_id, reservation))}><Edit3 size={16} /></button> : null}
                </article>
              );
            })}
          </div>
      ...
      {focusedTable ? (
        <div className="reserve-sheet-backdrop-v2" onMouseDown={() => setFocusedTable(null)}>
          <section className="reserve-table-sheet-v2" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="reserve-focused-list-v2">
              {focusedBookings.length === 0 ? <div className="reserve-empty-v2"><Check size={28} /><strong>Стіл вільний</strong><span>На вибрану дату немає бронювань.</span></div> : focusedBookings.map((reservation) => (
                <button type="button" key={reservation.id} onClick={() => { setEditor(createEditor(selectedDate, reservation.hall_id, reservation.table_id, reservation)); setFocusedTable(null); }}>
                  <time>{shortTime(reservation.reserved_time)}</time>
                  <span><strong>{reservation.guest_name}</strong><small>{reservation.guests} гостей · {STATUS_META[reservation.status].label}</small></span>
                  <Edit3 size={15} />
                </button>
              ))}
            </div>
      ...
      {editor ? (
        <div className="reserve-sheet-backdrop-v2" onMouseDown={() => !saving && setEditor(null)}>
          <section className="reserve-editor-sheet-v2" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="reserve-editor-grid-v2">
              <label><span>Зал</span><select value={editor.hallId} onChange={(event) => { const hallId = Number(event.target.value); const first = tables.filter((table) => table.hall_id === hallId).sort(naturalTableSort)[0]; setEditor({ ...editor, hallId, tableId: first?.id || 0, mergedTables: [] }); }}>{halls.map((hall) => <option value={hall.id} key={hall.id}>{hall.name}</option>)}</select></label>
              <label><span>Основний стіл</span><select value={editor.tableId} onChange={(event) => { const tableId = Number(event.target.value); const label = editorTables.find((table) => table.id === tableId)?.label; setEditor({ ...editor, tableId, mergedTables: editor.mergedTables.filter((item) => item !== label) }); }}>{editorTables.map((table) => <option value={table.id} key={table.id}>{table.label} · {table.capacity} ос.</option>)}</select></label>
              ...
              <label><span>Тривалість</span><select value={editor.durationMinutes} onChange={(event) => setEditor({ ...editor, durationMinutes: Number(event.target.value) })}>{[60, 90, 120, 150, 180, 240, 300].map((minutes) => <option value={minutes} key={minutes}>{minutes < 60 ? `${minutes} хв` : `${Math.floor(minutes / 60)} год${minutes % 60 ? ` ${minutes % 60} хв` : ''}`}</option>)}</select></label>
            ...
            <div className="reserve-merge-editor-v2">
              <div><strong>Об’єднати столи</strong><span>Загальна місткість: {editorCapacity} ос.</span></div>
              <div>{editorTables.filter((table) => table.id !== editor.tableId).map((table) => { const active = editor.mergedTables.includes(table.label); return <button type="button" key={table.id} className={active ? 'is-active' : ''} onClick={() => setEditor({ ...editor, mergedTables: active ? editor.mergedTables.filter((label) => label !== table.label) : [...editor.mergedTables, table.label] })}>{active ? <Check size={13} /> : null}{table.label}<span>{table.capacity}</span></button>; })}</div>
            </div>
            ...
            <div className="reserve-editor-actions-v2">
              {editor.id ? <button type="button" className="is-delete" onClick={() => void deleteReservation()} disabled={saving}><Trash2 size={17} /> Видалити</button> : <span />}
              <button type="button" className="is-save" onClick={() => void saveReservation()} disabled={saving}>{saving ? <RefreshCw size={17} className="is-spinning" /> : <Check size={17} />} Зберегти</button>
            </div>
          </section>
        </div>
      ) : null}
