...
const Horoscope = {
  ...
  async _callClaude(zodiac, role) {
    ...
    console.log('[Horo] HTTP status:', resp.status);
    if (!resp.ok) throw new Error('Edge Function error: ' + resp.status);
    const envelope = await resp.json();
    console.log('[Horo] envelope keys:', Object.keys(envelope));
    console.log('[Horo] envelope.ok:', envelope.ok, '| envelope.text:', (envelope.text||'').slice(0,80), '| envelope.error:', envelope.error);
    if (!envelope.ok) throw new Error(envelope.error || 'generate_horoscope failed');
    const raw = envelope.text || (envelope.data?.content||[]).map(c=>c.text||'').join('');
    console.log('[Horo] raw length:', raw.length, '| first 120:', raw.slice(0,120));
    const text = raw.replace(/```json|```/g,'').trim();
    const sanitized = text.replace(/[⺀-⿿　-鿿가-퟿切-﫿︰-﹏]/g, '');
    try {
      return JSON.parse(sanitized);
    } catch(e) {
      console.error('[Horo] JSON parse error. sanitized:', sanitized.slice(0,200));
  ...
  async _callClaudeOffDay(zodiac) {
    ...
    console.log('[HoroOff] HTTP status:', resp.status);
    if (!resp.ok) throw new Error('Edge Function error: ' + resp.status);
    const envelope = await resp.json();
    console.log('[HoroOff] envelope keys:', Object.keys(envelope));
    console.log('[HoroOff] envelope.ok:', envelope.ok, '| envelope.text:', (envelope.text||'').slice(0,80));
    if (!envelope.ok) throw new Error(envelope.error || 'generate_horoscope failed');
    const raw = envelope.text || (envelope.data?.content||[]).map(c=>c.text||'').join('');
    const text = raw.replace(/```json|```/g,'').trim();
    const sanitized = text.replace(/[⺀-⿿　-鿿가-퟿切-﫿︰-﹏]/g, '');
    try {
      ...
      console.error('[HoroOff] JSON parse error. sanitized:', sanitized.slice(0,200));
      throw new Error('Не вдалося розпарсити відповідь від AI');
    }
  },
