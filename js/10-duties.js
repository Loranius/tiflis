...
{"assignments": [{"dutyIndex": 0, "waiterName": "Іра"}]}`;
      ...
      if (isFullDaily) {
        ...
        // Зони — Claude може повернути zoneIndex або dutyIndex, підтримуємо обидва
        (parsed.zones || []).forEach(a => {
          const waiter = matchWaiter(a.waiterName);
          const idx    = a.zoneIndex !== undefined ? a.zoneIndex : a.dutyIndex;
          const zone   = zonesList[idx];
          if (waiter && zone !== undefined) matchedZones.push({ dutyIndex: idx, waiter, duty: zone, _list: 'zones' });
        });
      } else {
      ...
      Duties._showPhotoOverlay(`
        <div style="padding:20px">
          ...
          <div style="display:flex;gap:8px">
            <button class="btn btn-gold" style="flex:1" data-action="apply">
              ✅ Застосувати
            </button>
  ...
  _showPhotoOverlay(html) {
    ...
    // Використовуємо createElement замість innerHTML щоб onclick працював на мобільному
    ...
    // Призначаємо обробники через addEventListener (надійно на всіх платформах)
    const applyBtn = inner.querySelector('[data-action="apply"]');
    if (applyBtn) applyBtn.addEventListener('click', () => Duties._applyPendingPhoto());

    inner.querySelectorAll('[data-action="close"]').forEach(btn => {
      btn.addEventListener('click', () => Duties._closePhotoOverlay());
    });

    // Очищаємо попередній контент і вставляємо новий
    ov.innerHTML = '';
